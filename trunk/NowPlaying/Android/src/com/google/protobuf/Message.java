...
public interface Message {
  /**
   * Get the message's type's descriptor.  This differs from the {@code getDescriptor()} method of generated message
   * classes in that this method is an abstract method of the {@code Message} interface whereas {@code getDescriptor()}
   ...
   * abstract method of the {@code Message} interface whereas {@code getDefaultInstance()} is a static method of a
   ...
   * ("required" or "optional") field is set iff hasField() returns true for that field.  A "repeated" field is set iff
   * getRepeatedFieldSize() is greater than zero.  The values are exactly what would be returned by calling {@link
   * #getField(Descriptors.FieldDescriptor)} for each field.  The map is guaranteed to be a sorted map, so iterating
   * over it will return fields in order by field number.
   */
   ...
   *
   * @throws IllegalArgumentException The field is a repeated field, or {@code field.getContainingType() !=
   ...
   * accessor method corresponding to the field.
   *
   ...
   * fields, the EnumValueDescriptor for the value is returend. For embedded message fields, the sub-message is
   * returned.
   *
   * @throws IllegalArgumentException The field is not a repeated field, or {@code field.getContainingType() !=
   *                                  getDescriptorForType()}.
   ...
   * message of the same type (as defined by {@code getDescriptorForType()}) and has identical values for all of its
   * fields.
   *
   ...
   * <tt>getDescriptor().hashCode() ^ map.hashCode()</tt>, where <tt>map</tt> is a map of field numbers to field
   * values.
   *
  ...
  public static interface Builder extends Cloneable {
     ...
     * (i.e. {@code getDescriptorForType() == other.getDescriptorForType()}).
     * <p/>
     * Merging occurs as follows.  For each field:<br> * For singular primitive fields, if the field is set in {@code
     * other}, then {@code other}'s value overwrites the value in this message.<br> * For singular message fields, if
     * the field is set in {@code other}, it is merged into the corresponding sub-message of this message using the same
     * merging rules.<br> * For repeated fields, the elements in {@code other} are concatenated with the elements in
     * this message.
     * <p/>
     ...
     * build()}, {@code clone()} it first.
     *
     * @throws UninitializedMessageException The message is missing one or more required fields (i.e. {@link
     *                                       #isInitialized()} returns false). Use {@link #buildPartial()} to bypass
     *                                       this check.
     */
    Message build();
     ...
     * <p/>
     * <p>Warning:  This does not verify that all required fields are present in the input message.  If you call {@link
     * #build()} without setting all required fields, it will throw an {@link UninitializedMessageException}, which is a
     * {@code RuntimeException} and thus might not be caught.  There are a few good ways to deal with this: <ul>
     * <li>Call {@link #isInitialized()} to verify that all required fields are set before building. <li>Parse the
     * message separately using one of the static {@code parseFrom} methods, then use {@link #mergeFrom(Message)} to
     * merge it with this one.  {@code parseFrom} will throw an {@link InvalidProtocolBufferException} (an {@code
     * IOException}) if some required fields are missing. <li>Use {@code buildPartial()} to build, which ignores missing
     * required fields. </ul>
     * <p/>
     * <p>Note:  The caller should call {@link CodedInputStream#checkLastTagWas(int)} after calling this to verify that
     ...
     * Either way, the returned map is itself unmodifiable.
     */
    Map<Descriptors.FieldDescriptor, Object> getAllFields();
     ...
     * i.e. the same type that {@link Message#getRepeatedField(Descriptors.FieldDescriptor,int)} would return.
     *
     * @throws IllegalArgumentException The field is not a repeated field, or {@code field.getContainingType() !=
     *                                  getDescriptorForType()}.
     */
     ...
     * wrapper around {@link #mergeFrom(CodedInputStream)}.  Note that this method always reads the <i>entire</i> input
     * (unless it throws an exception).  If you want it to stop earlier, you will need to wrap your input in some
     * wrapper stream that limits reading.  Despite usually reading the entire input, this does not close the stream.
     */
