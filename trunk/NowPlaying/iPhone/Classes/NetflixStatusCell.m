...
- (void) initialize {
    ...
    if (!status.queue.isAtHomeQueue && status.description.length > 0) {
        ...
        deleteFrame.origin.x = moveFrame.size.width + 15;
        deleteImageView.frame = deleteFrame;
        
        CGRect frame = CGRectMake(0, 0, deleteFrame.origin.x + deleteFrame.size.width + 5, MAX(deleteFrame.size.height, moveFrame.size.height));
