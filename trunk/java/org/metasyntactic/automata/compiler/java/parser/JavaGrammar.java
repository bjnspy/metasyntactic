...
public class JavaGrammar extends PackratGrammar<JavaToken.Type> {
  ...
  static {
    ...
    javaStartRule = new Rule("CompilationUnit", sequence(optional(variable("PackageDeclaration")),
  ...
  private static void addArrays(Set<Rule> rules) {
    rules.add(new Rule("ArrayInitializer", sequence(token(LeftCurlySeparatorToken.instance), delimitedOptionalList(
        variable("VariableInitializer"), token(CommaSeparatorToken.instance)), optional(
        token(CommaSeparatorToken.instance)), token(RightCurlySeparatorToken.instance))));
  ...
  private static void addTypes(Set<Rule> rules) {
    rules.add(new Rule("TypeParameters", sequence(token(LessThanOperatorToken.instance), delimitedList(
        variable("TypeParameter"), token(CommaSeparatorToken.instance)), token(GreaterThanOperatorToken.instance))));

    rules.add(new Rule("TypeParameter", sequence(identifier(), optional(variable("TypeBound")))));

    rules.add(new Rule("TypeBound", sequence(token(ExtendsKeywordToken.instance), delimitedList(
        variable("ClassOrInterfaceType"), token(BitwiseAndOperatorToken.instance)))));
  ...
  private static void addEnumDeclaration(Set<Rule> rules) {
    rules.add(new Rule("EnumDeclaration", sequence(variable("Modifiers"), token(EnumKeywordToken.instance),
                                                   identifier(), optional(variable("Interfaces")),
                                                   variable("EnumBody"))));

    rules.add(new Rule("EnumBody", sequence(token(LeftCurlySeparatorToken.instance), delimitedOptionalList(
        variable("EnumConstant"), token(CommaSeparatorToken.instance)), optional(token(CommaSeparatorToken.instance)),
                                                                        optional(
                                                                            token(SemicolonSeparatorToken.instance)),
                                                                        repetition(variable("ClassBodyDeclaration")),
                                                                        token(RightCurlySeparatorToken.instance))));

    rules.add(new Rule("EnumConstant", sequence(repetition(variable("Annotation")), identifier(),
  ...
  private static void addAnnotationDeclaration(Set<Rule> rules) {
    rules.add(new Rule("AnnotationDeclaration", sequence(variable("Modifiers"), token(AtSeparatorToken.instance),
                                                         token(InterfaceKeywordToken.instance), identifier(),
                                                         variable("AnnotationBody"))));

    rules.add(new Rule("AnnotationBody", sequence(token(LeftCurlySeparatorToken.instance),
                                                  repetition(variable("AnnotationElementDeclaration")),
                                                  token(RightCurlySeparatorToken.instance))));

    rules.add(new Rule("AnnotationElementDeclaration", choice(variable("AnnotationDefaultDeclaration"),
  ...
  private static void addMemberDeclarations(Set<Rule> rules) {
    rules.add(new Rule("ClassOrInterfaceMemberDeclaration", choice(variable("FieldDeclaration"),
    ...
    rules.add(new Rule("ConstructorDeclaration", sequence(variable("Modifiers"), optional(variable("TypeParameters")),
                                                          ...
                                                          token(RightParenthesisSeparatorToken.instance),
                                                          optional(variable("Throws")), variable("Block"))));

    rules.add(new Rule("FieldDeclaration", sequence(variable("Modifiers"), variable("Type"), delimitedList(
    ...
    rules.add(new Rule("Throws", sequence(token(ThrowsKeywordToken.instance), delimitedList(
        variable("ClassOrInterfaceType"), token(CommaSeparatorToken.instance)))));
  }
  ...
  private static void addExpressions(Set<Rule> rules) {
    ...
    rules.add(new Rule("AssignmentOperator", choice(token(EqualsOperatorToken.instance),
    ...
    rules.add(new Rule("InfixOperator", choice(token(LogicalOrOperatorToken.instance),
                                               ...
                                               token(LeftShiftOperatorToken.instance), sequence(
        token(GreaterThanOperatorToken.instance), token(GreaterThanOperatorToken.instance),
        token(GreaterThanOperatorToken.instance)), sequence(token(GreaterThanOperatorToken.instance),
    ...
    rules.add(new Rule("Expression3", choice(variable("PrefixExpression"), variable("PossibleCastExpression"),
                                             variable("PrimaryExpression"))));

    rules.add(new Rule("PrefixExpression", sequence(variable("PrefixOperator"), variable("Expression3"))));

    rules.add(new Rule("PrefixOperator", choice(token(IncrementOperatorToken.instance),
    ...
    rules.add(new Rule("PostfixOperator",
                       choice(token(IncrementOperatorToken.instance), token(DecrementOperatorToken.instance))));
    ...
    rules.add(new Rule("ValueExpression", choice(variable("ParenthesizedExpression"), variable("MethodInvocation"),
                                                 ...
                                                 token(ThisKeywordToken.instance), token(SuperKeywordToken.instance),
                                                 variable("ClassAccess"), type(LiteralToken.class), identifier(),
                                                 variable("CreationExpression"))));

    rules.add(new Rule("ClassAccess", sequence(variable("Type"), token(DotSeparatorToken.instance),
    ...
    rules.add(new Rule("ParenthesizedExpression", sequence(token(LeftParenthesisSeparatorToken.instance),
                                                           variable("Expression"),
                                                           token(RightParenthesisSeparatorToken.instance))));

    rules.add(new Rule("MethodInvocation",
                       sequence(optional(variable("NonWildcardTypeArguments")), identifier(), variable("Arguments"))));

    rules.add(new Rule("ThisConstructorInvocation", sequence(token(ThisKeywordToken.instance), variable("Arguments"))));

    rules.add(
        new Rule("SuperConstructorInvocation", sequence(token(SuperKeywordToken.instance), variable("Arguments"))));

    rules.add(new Rule("CreationExpression",
                       choice(variable("ObjectCreationExpression"), variable("ArrayCreationExpression"))));

    rules.add(new Rule("ObjectCreationExpression", sequence(token(NewKeywordToken.instance),
  ...
  private static void addStatements(Set<Rule> rules) {
    rules.add(new Rule("Block", sequence(token(LeftCurlySeparatorToken.instance),
                                         repetition(variable("BlockStatement")),
                                         token(RightCurlySeparatorToken.instance))));

    rules.add(new Rule("BlockStatement", choice(variable("LocalVariableDeclarationStatement"),
                                                variable("ClassDeclaration"), variable("Statement"))));

    rules.add(new Rule("LocalVariableDeclarationStatement",
                       sequence(variable("LocalVariableDeclaration"), token(SemicolonSeparatorToken.instance))));
    ...
    rules.add(new Rule("Statement", choice(variable("Block"), variable("EmptyStatement"),
    ...
    rules.add(new Rule("LabeledStatement",
                       sequence(identifier(), token(ColonOperatorToken.instance), variable("Statement"))));

    rules.add(
        new Rule("ExpressionStatement", sequence(variable("Expression"), token(SemicolonSeparatorToken.instance))));
    ...
    rules.add(new Rule("SwitchBlockStatementGroup",
                       sequence(oneOrMore(variable("SwitchLabel")), oneOrMore(variable("BlockStatement")))));

    rules.add(new Rule("SwitchLabel", choice(variable("CaseSwitchLabel"), variable("DefaultSwitchLabel"))));

    rules.add(new Rule("CaseSwitchLabel", sequence(token(CaseKeywordToken.instance), variable("Expression"),
                                                   token(ColonOperatorToken.instance))));

    rules.add(new Rule("DefaultSwitchLabel",
                       sequence(token(DefaultKeywordToken.instance), token(ColonOperatorToken.instance))));

    rules.add(new Rule("WhileStatement", sequence(token(WhileKeywordToken.instance),
                                                  token(LeftParenthesisSeparatorToken.instance), variable("Expression"),
                                                  token(RightParenthesisSeparatorToken.instance),
                                                  variable("Statement"))));

    rules.add(new Rule("DoStatement", sequence(token(DoKeywordToken.instance), variable("Statement"),
    ...
    rules.add(new Rule("BasicForStatement", sequence(token(ForKeywordToken.instance),
                                                     token(LeftParenthesisSeparatorToken.instance),
    ...
    rules.add(new Rule("ForInitializer", choice(variable("LocalVariableDeclaration"), delimitedList(
        variable("Expression"), token(CommaSeparatorToken.instance)))));

    rules.add(new Rule("ForUpdate", delimitedList(variable("Expression"), token(CommaSeparatorToken.instance))));

    rules.add(new Rule("EnhancedForStatement", sequence(token(ForKeywordToken.instance),
                                                        token(LeftParenthesisSeparatorToken.instance),
                                                        variable("Modifiers"), variable("Type"), identifier(),
                                                        token(ColonOperatorToken.instance), variable("Expression"),
                                                        token(RightParenthesisSeparatorToken.instance),
                                                        variable("Statement"))));

    rules.add(new Rule("BreakStatement", sequence(token(BreakKeywordToken.instance), optional(identifier()),
                                                  token(SemicolonSeparatorToken.instance))));
    ...
    rules.add(new Rule("SynchronizedStatement", sequence(token(SynchronizedKeywordToken.instance),
                                                         token(LeftParenthesisSeparatorToken.instance),
                                                         variable("Expression"),
                                                         token(RightParenthesisSeparatorToken.instance),
                                                         variable("Block"))));
    ...
    rules.add(
        new Rule("TryStatement", choice(variable("TryStatementWithFinally"), variable("TryStatementWithoutFinally"))));
    ...
    rules.add(new Rule("TryStatementWithFinally", sequence(token(TryKeywordToken.instance), variable("Block"),
                                                           ...
                                                           token(FinallyKeywordToken.instance), variable("Block"))));
    ...
    rules.add(new Rule("CatchClause", sequence(token(CatchKeywordToken.instance),
                                               token(LeftParenthesisSeparatorToken.instance),
                                               variable("FormalParameter"),
                                               token(RightParenthesisSeparatorToken.instance), variable("Block"))));
  }
  ...
  private static void addInterfaceDeclaration(Set<Rule> rules) {
    rules.add(new Rule("InterfaceDeclaration",
                       choice(variable("NormalInterfaceDeclaration"), variable("AnnotationDeclaration"))));

    rules.add(new Rule("NormalInterfaceDeclaration", sequence(variable("Modifiers"),
                                                              token(InterfaceKeywordToken.instance), identifier(),
                                                              optional(variable("TypeParameters")),
                                                              optional(variable("ExtendsInterfaces")),
                                                              variable("ClassOrInterfaceBody"))));

    rules.add(new Rule("ExtendsInterfaces", sequence(token(ExtendsKeywordToken.instance), delimitedList(
        variable("ClassOrInterfaceType"), token(CommaSeparatorToken.instance)))));

    rules.add(new Rule("ClassOrInterfaceBody", sequence(token(LeftCurlySeparatorToken.instance),
  ...
  private static void addClassDeclaration(Set<Rule> rules) {
    ...
    rules.add(new Rule("NormalClassDeclaration", sequence(variable("Modifiers"), token(ClassKeywordToken.instance),
                                                          identifier(), optional(variable("TypeParameters")),
                                                          optional(variable("Super")), optional(variable("Interfaces")),
                                                          variable("ClassBody"))));

    rules.add(new Rule("Modifiers", repetition(variable("Modifier"))));

    rules.add(new Rule("Modifier", choice(variable("Annotation"), token(PublicKeywordToken.instance),
    ...
    rules.add(new Rule("Interfaces", sequence(token(ImplementsKeywordToken.instance), delimitedList(
        variable("ClassOrInterfaceType"), token(CommaSeparatorToken.instance)))));

    rules.add(new Rule("ClassBody", sequence(token(LeftCurlySeparatorToken.instance),
                                             repetition(variable("ClassBodyDeclaration")),
                                             token(RightCurlySeparatorToken.instance))));
  ...
  */
  ...
  private static void addCompilationUnit(Set<Rule> rules) {
    rules.add(new Rule("PackageDeclaration", sequence(repetition(variable("Annotation")),
    ...
    rules.add(new Rule("ImportDeclaration", choice(variable("SingleTypeImportDeclaration"),
                                                   variable("TypeImportOnDemandDeclaration"),
                                                   variable("SingleStaticImportDeclaration"),
                                                   variable("StaticImportOnDemandDeclaration"))));

    rules.add(new Rule("SingleTypeImportDeclaration", sequence(token(ImportKeywordToken.instance),
                                                               variable("QualifiedIdentifier"),
                                                               token(SemicolonSeparatorToken.instance))));

    rules.add(new Rule("TypeImportOnDemandDeclaration", sequence(token(ImportKeywordToken.instance),
                                                                 variable("QualifiedIdentifier"),
                                                                 token(DotSeparatorToken.instance),
                                                                 token(TimesOperatorToken.instance),
                                                                 token(SemicolonSeparatorToken.instance))));

    rules.add(new Rule("SingleStaticImportDeclaration", sequence(token(ImportKeywordToken.instance),
                                                                 token(StaticKeywordToken.instance),
                                                                 variable("QualifiedIdentifier"),
                                                                 token(SemicolonSeparatorToken.instance))));

    rules.add(new Rule("StaticImportOnDemandDeclaration", sequence(token(ImportKeywordToken.instance),
                                                                   token(StaticKeywordToken.instance),
                                                                   variable("QualifiedIdentifier"),
                                                                   token(DotSeparatorToken.instance),
                                                                   token(TimesOperatorToken.instance),
    ...
    rules.add(new Rule("TypeDeclaration", choice(variable("ClassDeclaration"), variable("InterfaceDeclaration"),
                                                 token(SemicolonSeparatorToken.instance))));
  }

  private static void addAnnotation(Set<Rule> rules) {
    rules.add(new Rule("Annotation", choice(variable("NormalAnnotation"), variable("SingleElementAnnotation"),
    ...
    rules.add(new Rule("ElementValuePair",
                       sequence(identifier(), token(EqualsOperatorToken.instance), variable("ElementValue"))));

    rules.add(new Rule("SingleElementAnnotation", sequence(token(AtSeparatorToken.instance),
                                                           variable("QualifiedIdentifier"),
                                                           token(LeftParenthesisSeparatorToken.instance),
                                                           variable("ElementValue"),
                                                           token(RightParenthesisSeparatorToken.instance))));
    ...
    rules.add(
        new Rule("MarkerAnnotation", sequence(token(AtSeparatorToken.instance), variable("QualifiedIdentifier"))));

    rules.add(new Rule("ElementValue", choice(variable("Annotation"), variable("Expression"),
