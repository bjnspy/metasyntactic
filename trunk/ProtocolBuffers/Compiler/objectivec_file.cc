...
FileGenerator::FileGenerator(const FileDescriptor* file)
  : file_(file),
    objectivec_package_(FileObjectiveCPackage(file)),
...
void FileGenerator::GenerateForwardDeclarations(io::Printer* printer) {
    for (int i = 0; i < file_->enum_type_count(); i++) {
      EnumGenerator(file_->enum_type(i)).GenerateForwardDeclarations(printer);
    }
    for (int i = 0; i < file_->service_count(); i++) {
      ServiceGenerator(file_->service(i)).GenerateForwardDeclarations(printer);
    }
    for (int i = 0; i < file_->message_type_count(); i++) {
      MessageGenerator(file_->message_type(i)).GenerateForwardDeclarations(printer);
    }
}

void FileGenerator::GenerateSource(io::Printer* printer) {
  FileGenerator file_generator(file_);
  string header_file = file_generator.classname() + ".pb.h";
