...
public class PythonLexicalSpecification extends PackratGrammar<PythonToken.Type> {
  ...
  public static Rule WHITESPACE_RULE = new Rule("Whitespace", oneOrMore(choice(terminal(" "), terminal("\t"), terminal(
      "\f"))));

  public static Rule NEWLINE_RULE = new Rule("NewLine", choice("\r\n", "\r", "\n"));

  public static Rule LINE_CONTINUATION_RULE = new Rule("LineContinuation", terminal("\\"));

  public static Rule COMMENT_RULE = new Rule("Comment", sequence(terminal("#"),
                                                                 // keep on reading characters until we hit a newline or the end of the input
                                                                 repetition(sequence(not(choice(variable("NewLine"),
                                                                                                endOfStream())),
                                                                                     anyCharacter()))));

  public static Rule KEYWORD_RULE = new Rule("Keyword", choice(KeywordToken.getKeywords()));

  public static Rule FLOATING_POINT_LITERAL_RULE = new Rule("FloatingPointLiteral", choice(variable(
      "PointFloatingPointLiteral"), variable("ExponentFloatingPointLiteral")));

  public static Rule INTEGER_LITERAL_RULE = new Rule("IntegerLiteral", sequence(choice(variable("OctalInteger"),
                                                                                       variable("HexInteger"), variable(
      "DecimalInteger")), optional(choice("l", "L"))));

  public static Rule IMAGINARY_NUMBER_LITERAL_RULE = new Rule("ImaginaryNumberLiteral", sequence(choice(variable(
      "FloatingPointLiteral"), variable("IntegerPart")), choice("j", "J")));

  public static Rule STRING_LITERAL_RULE = new Rule("StringLiteral", sequence(optional(choice("r", "u", "ur", "R", "U",
                                                                                              "UR", "Ur", "uR")),
                                                                              choice(variable("LongString"), variable(
                                                                                  "ShortString"))));

  public static Rule IDENTIFIER_RULE = new Rule("Identifier", sequence(choice(variable("Letter"), terminal("_")),
                                                                       repetition(choice(variable("Letter"), variable(
                                                                           "Digit"), terminal("_")))));

  public static Rule OPERATOR_RULE = new Rule("Operator", choice(OperatorToken.getOperators()));

  public static Rule DELIMITER_RULE = new Rule("Delimiter", choice(DelimiterToken.getDelimiters()));
  ...
  private static Set<Rule> makeRules(Set<Rule> rules) {
    ...
    rules.add(new Rule("InputElement", choice(variable("Whitespace"), variable("NewLine"), variable("LineContinuation"),
                                              variable("Comment"), variable("Keyword"), variable("Literal"), variable(
        "Identifier"), variable("Operator"), variable("Delimiter"), variable("Error"))));
  ...
  private static void identifier(Set<Rule> rules) {
    ...
    rules.add(new Rule("Letter", choice(range('a', 'z'), range('A', 'Z'))));

    rules.add(new Rule("Digit", range('0', '9')));
  }
  ...
  private static void literals(Set<Rule> rules) {
    ...
    rules.add(new Rule("Literal", choice(variable("FloatingPointLiteral"), variable("IntegerLiteral"), variable(
        "StringLiteral"), variable("ImaginaryNumberLiteral"))));
  ...
  private static void stringLiterals(Set<Rule> rules) {
    ...
    rules.add(new Rule("ShortString", choice(sequence(terminal("'"), repetition(variable("SingleQuoteShortStringItem")),
                                                      terminal("'")), sequence(terminal("\""), repetition(variable(
        "DoubleQuoteShortStringItem")), terminal("\"")))));

    rules.add(new Rule("LongString", choice(sequence(terminal("'''"), repetition(variable("SingleQuoteLongStringItem")),
                                                     terminal("'''")), sequence(terminal("\"\"\""), repetition(variable(
        "DoubleQuoteLongStringItem")), terminal("\"\"\"")))));

    rules.add(new Rule("SingleQuoteShortStringItem", choice(variable("SingleQuoteStringCharacter"), variable(
        "EscapeSequence"))));

    rules.add(new Rule("DoubleQuoteShortStringItem", choice(variable("DoubleQuoteStringCharacter"), variable(
        "EscapeSequence"))));

    rules.add(new Rule("SingleQuoteStringCharacter", sequence(not(choice(terminal("\\"), variable("NewLine"), terminal(
        "'"))), anyCharacter())));

    rules.add(new Rule("DoubleQuoteStringCharacter", sequence(not(choice(terminal("\\"), variable("NewLine"), terminal(
        "\""))), anyCharacter())));

    rules.add(new Rule("SingleQuoteLongStringItem", choice(variable("SingleQuoteLongStringCharacter"), variable(
        "EscapeSequence"))));

    rules.add(new Rule("DoubleQuoteLongStringItem", choice(variable("DoubleQuoteLongStringCharacter"), variable(
        "EscapeSequence"))));

    rules.add(new Rule("SingleQuoteLongStringCharacter", sequence(not(choice(terminal("\\"), terminal("'''"))),
                                                                  anyCharacter())));

    rules.add(new Rule("DoubleQuoteLongStringCharacter", sequence(not(choice(terminal("\\"), terminal("\"\"\""))),
                                                                  anyCharacter())));

    rules.add(new Rule("EscapeSequence", sequence(terminal("\\"), anyCharacter())));
  }

  private static void floatingPointLiterals(Set<Rule> rules) {
    rules.add(FLOATING_POINT_LITERAL_RULE);

    rules.add(new Rule("PointFloatingPointLiteral", choice(sequence(optional(variable("IntegerPart")), variable(
        "Fraction")), sequence(variable("IntegerPart"), terminal(".")))));

    rules.add(new Rule("ExponentFloatingPointLiteral", sequence(choice(variable("IntegerPart"), variable(
        "PointFloatingPointLiteral")), variable("Exponent"))));

    rules.add(new Rule("IntegerPart", oneOrMore(variable("Digit"))));

    rules.add(new Rule("Fraction", sequence(terminal("."), variable("IntegerPart"))));

    rules.add(new Rule("Exponent", sequence(choice(terminal("e"), terminal("E")), optional(choice(terminal("+"),
                                                                                                  terminal("-"))),
  ...
  private static void integerLiterals(Set<Rule> rules) {
    ...
    rules.add(new Rule("DecimalInteger", choice(sequence(range('1', '9'), repetition(variable("Digit"))), terminal(
        "0"))));

    rules.add(new Rule("OctalInteger", sequence(terminal("0"), oneOrMore(range('0', '7')))));

    rules.add(new Rule("HexInteger", sequence(terminal("0"), choice("x", "X"), oneOrMore(variable("HexDigit")))));
